 ...
 * @author endeleya
 */
public class CommandeService implements CommandeStorage {

    @Override
    public boolean isExists(String uid) {
        String jpql = "SELECT CASE WHEN COUNT(c) > 0 THEN TRUE ELSE FALSE END "
                + "FROM Commande c WHERE c.uid = :id";
    ...
    @Override
    public void deleteCommande(Commande cat) {
        if (ManagedSessionFactory.isEmbedded()) {
            ManagedSessionFactory.submitWrite(em -> {
    ...
    public boolean isExists(String uid, LocalDateTime atime) {
        ...
        sb.append("SELECT * FROM commande p WHERE p.uid = ? AND p.updated_at = ?");
        if (ManagedSessionFactory.isEmbedded()) {
        ...
        Query query = ManagedSessionFactory.getEntityManager().createNativeQuery(sb.toString(), Commande.class);
        query.setParameter(1, uid);
        query.setParameter(2, atime);
        List<Commande> result = query.getResultList();
        return !result.isEmpty();
