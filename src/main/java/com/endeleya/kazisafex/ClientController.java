...
public class ClientController implements Initializable {
    ...
    public void addClientOrganisation(ClientOrganisation liv) {
        ClientOrganisation l = ClientOrganisationDelegate.findClientOrganisation(liv.getUid());//db.findByUid(ClientOrganisation.class, liv.getUid());
        if (l == null) {
    ...
    public void addClientAppart(ClientAppartenir liv) {
        ClientAppartenir l = ClientAppartenirDelegate.findClientAppartenir(liv.getUid());// db.findByUid(ClientAppartenir.class, liv.getUid());
        if (l == null) {
    ...
    public void setUp(Entreprise eze, String token, String region) {
        ...
        cbx_type_client_.setItems(FXCollections.observableArrayList(bundle.getString("consumer"), bundle.getString("wholesaler"), bundle.getString("detailor"), bundle.getString("subscriber")));
        cbx_type_client_.getSelectionModel().selectFirst();
        cbxdomainorg.setItems(FXCollections.observableArrayList("Santé", "Service de guardiennage", "Informatique", "ONG", "Telecomunication", "Bancaire", "Commerce général", "Agro-Business", "Minier", "Agro-alimentaire", "Service public", "Autres"));
        ...
        tf_search_client.textProperty().addListener(new ChangeListener<String>() {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                ...
                for (Client c : lsclts) {
                    ...
                    String oc = c.getAdresse() + " " + c.getEmail() + " " + c.getNomClient() + " " + c.getPhone() + " " + typecli;
        ...
        listOrganiz.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<ClientOrganisation>() {
            @Override
            public void changed(ObservableValue<? extends ClientOrganisation> observable, ClientOrganisation oldValue, ClientOrganisation newValue) {
                if (newValue != null) {
        ...
        m2.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                if (choosenClient != null) {
                    Alert alertdlg = new Alert(Alert.AlertType.CONFIRMATION, bundle.getString("sure2deleteclient"), ButtonType.YES, ButtonType.CANCEL);
            ...
            public void changed(ObservableValue<? extends Client> observable, Client oldValue, Client newValue) {
                if (newValue != null) {
                    ...
                    String type = choosenClient.getTypeClient().equals("#0") ? bundle.getString("consumer")
                            ...
                            : choosenClient.getTypeClient().equals("#3") ? bundle.getString("subscriber") : bundle.getString("consumer");
                    if (choosenClient.getTypeClient().equals("#3")) {
                        savedOrgs.setDisable(false);
                        btnaddorg.setDisable(false);
                        List<ClientAppartenir> clientApps = ClientAppartenirDelegate.findAppartenanceFor(choosenClient.getUid());// db.findClientTools(ClientAppartenir.class, choosenClient.getUid());
                        orgs.clear();
                        for (ClientAppartenir clientApp : clientApps) {
                            ClientOrganisation orga = ClientOrganisationDelegate.findClientOrganisation(clientApp.getClientOrganisationId().getUid());
                            orgs.add(orga);
                        }
    ...
    private void saveClient(Event e) {
        if (tf_adresse_client_.getText().isEmpty() || tf_nom_client_.getText().isEmpty() || tf_phone_client_.getText().isEmpty()) {
            MainUI.notify(null, bundle.getString("error"), bundle.getString("fillfield"), 5, "error");
            return;
        }
        List<Client> lclt = ClientDelegate.findClientByPhone(tf_phone_client_.getText());// db.findWithAndClause(Client.class, new String[]{"phone"}, new String[]{tf_phone_client_.getText()});
        if (!lclt.isEmpty()) {
        ...
        if (clients.isEmpty()) {
            ...
            if (indexorg == 1 && savedOrgs.getValue() != null) {
                ...
                cap.setDateAppartenir(new Date());
    ...
    private void configList() {
            ...
            protected void updateItem(Client item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    String typecli = item.getTypeClient().equals("#0") ? bundle.getString("consumer")
                            ...
                            : item.getTypeClient().equals("#3") ? bundle.getString("subscriber") : bundle.getString("consumer");
            ...
            protected void updateItem(ClientOrganisation item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(item.getNomOrganisation() + ", " + item.getDomaineOrganisation() + " "
                            + "" + item.getAdresse() + "  " + (item.getWebsiteOrganisation() != null ? item.getWebsiteOrganisation() : ""));
                    imageView.setFitHeight(30);
                    imageView.setFitWidth(30);
                    imageView.setPreserveRatio(true);
                    imageView.setImage(new Image(ClientController.class.getResourceAsStream("/icons/office-building.png")));
                    setGraphic(imageView);
                }
            }
        ...
        savedOrgs.setConverter(new StringConverter<ClientOrganisation>() {
            ...
            public String toString(ClientOrganisation item) {
                return item == null ? null : item.getNomOrganisation() + ", " + item.getDomaineOrganisation() + " "
                        + "" + item.getAdresse() + " " + (item.getWebsiteOrganisation() != null ? item.getWebsiteOrganisation() : "");
            }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        mi.setOnAction((event) -> {
            if (organization != null) {
                if (role.equals(Role.Trader.name()) | role.contains(Role.Manager.name()) | role.contains(Role.Finance.name())) {
                    MainUI.floatDialog(tools.Constants.RELEVEE_DLG, 988, 598, token, kazisafe, entreprise, organization);
                } else {
                    MainUI.notify(null, "Erreur", "Vous n'avez pas assez des privileges pour acceder a cette option", 3, "error");
                }
            }
