...
public class WsClientEndpoint extends Endpoint {
    ...
    public void onOpen(Session s, EndpointConfig ec) {
        ...
        // Trigger catch-up sync on reconnection
        downsyncExecutor.submit(() -> {
            ...
            if (eUid != null) {
                ...
                System.err.println(
                    "[WS-CLIENT] Cannot trigger catch-up: eUid is missing."
                );
            }
        });
        ...
        s.addMessageHandler(BaseModel.class, (BaseModel obj) -> {
            downsyncExecutor.submit(() -> {
                data.SyncOutboxListener.runSuppressed(() -> {
                try {
                    String uid;
                    if (obj instanceof Category cat) {
                        ...
                        if (cat.getCount() == cat.getCounter()) {
                            if (!cat.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    cat.getCount() > 1
                                        ? "La categorie de produit " +
                                              cat.getDescritption() +
                                              " et " +
                                              (cat.getCount() - 1) +
                                              " autre categories sauvegardes "
                                        : "La categorie " +
                                              cat.getDescritption() +
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                Category exist = CategoryDelegate.findCategory(
                                    cat.getUid()
                                );
                                if (exist == null) {
                            ...
                            case Constants.ACTION_UPDATE -> CategoryDelegate.updateCategory(
                                cat
                            );
                            default -> CategoryDelegate.deleteCategory(cat);
                        }
                        System.out.println(
                            "Category object arrive dans onmessage"
                        );
                    } else if (obj instanceof Produit p) {
                        String action = p.getAction();
                        Category isExist1 = CategoryDelegate.findCategory(
                            p.getCategoryId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        if (
                            p.getCount() == p.getCounter() &&
                            !p.getAction().equals(Constants.ACTION_DELETE)
                        ) {
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Produit exist = ProduitDelegate.findProduit(
                                    p.getUid()
                                );
                                if (exist == null) {
                        ...
                        Produit isExist1 = ProduitDelegate.findProduit(
                            m.getProduitId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Mesure exist = MesureDelegate.findMesure(
                                    m.getUid()
                                );
                                if (exist == null) {
                        ...
                        switch (action) {
                            case
                                ...
                                FournisseurDelegate.syncFournisseurSafe(four);
                                break;
                            case Constants.ACTION_DELETE:
                        ...
                        javafx.application.Platform.runLater(() -> {
                            GoodstorageController.getInstance().populateSupplier(
                                action,
                                four
                            );
                        });
                        ...
                        Fournisseur isExist1 =
                            FournisseurDelegate.findFournisseur(
                                livr.getFournId().getUid()
                            );
                        if (isExist1 == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Livraison exist =
                                    LivraisonDelegate.findLivraison(
                                        livr.getUid()
                                    );
                                if (exist == null) {
                        ...
                        javafx.application.Platform.runLater(() -> {
                            GoodstorageController.getInstance().populateDelivery(
                                action,
                                livr
                            );
                        });
                        ...
                        Mesure isExist1 = MesureDelegate.findMesure(
                            stock.getMesureId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        uid = stock.getUid();
                        Produit isExist = ProduitDelegate.findProduit(
                            stock.getProductId().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Stocker exist = StockerDelegate.findStocker(
                                    stock.getUid()
                                );
                                if (exist == null) {
                        ...
                        javafx.application.Platform.runLater(() -> {
                            GoodstorageController.getInstance().populateStocker(
                                action,
                                stock
                            );
                        });
                        ...
                        Mesure isExist1 = MesureDelegate.findMesure(
                            destk.getMesureId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        Produit isExist = ProduitDelegate.findProduit(
                            destk.getProductId().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Destocker exist =
                                    DestockerDelegate.findDestocker(
                                        destk.getUid()
                                    );
                                if (exist == null) {
                        ...
                        Mesure isExist1 = MesureDelegate.findMesure(
                            recq.getMesureId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        Produit isExist = ProduitDelegate.findProduit(
                            recq.getProductId().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            ...
                            case Constants.ACTION_UPDATE -> RecquisitionDelegate.updateRecquisition(
                                recq
                            );
                            case Constants.ACTION_DELETE -> RecquisitionDelegate.deleteRecquisition(
                                recq
                            );
                            default -> {
                            }
                        ...
                        if (pv.getCount() == pv.getCounter()) {
                            if (!pv.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Le prix de la requisition " +
                                        isExist.getReference() +
                                        " bien configure",
                                    "Synchonisation"
                                );
                            }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                PrixDeVente exist =
                                    PrixDeVenteDelegate.findPrixDeVente(
                                        pv.getUid()
                                    );
                                if (exist == null) {
                            ...
                            case Constants.ACTION_UPDATE -> PrixDeVenteDelegate.updatePrixDeVente(
                                pv
                            );
                            case Constants.ACTION_DELETE -> PrixDeVenteDelegate.deletePrixDeVente(
                                pv
                            );
                            default -> {
                            }
                        ...
                        if (client.getCount() == client.getCounter()) {
                            if (!client.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    client.getCount() + " client enregistre",
                                    "Synchonisation"
                                );
                            }
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                ClientOrganisation exist =
                                    ClientOrganisationDelegate.findClientOrganisation(
                                        client.getUid()
                                    );
                                if (exist == null) {
                                    ClientOrganisationDelegate.updateClientOrganisation(
                                        client
                                    );
                                }
                            }
                            case Constants.ACTION_UPDATE -> ClientOrganisationDelegate.updateClientOrganisation(
                                client
                            );
                            case Constants.ACTION_DELETE -> ClientOrganisationDelegate.deleteClientOrganisation(
                                client
                            );
                            default -> {
                            }
                        ...
                        String action = client.getAction();
                        Client isExist = ClientDelegate.findClient(
                            client.getClientId().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        ClientOrganisation isExist1 =
                            ClientOrganisationDelegate.findClientOrganisation(
                                client.getClientOrganisationId().getUid()
                            );
                        if (isExist1 == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                ClientAppartenir exist =
                                    ClientAppartenirDelegate.findClientAppartenir(
                                        client.getUid()
                                    );
                                if (exist == null) {
                                    ClientAppartenirDelegate.updateClientAppartenir(
                                        client
                                    );
                                }
                                break;
                            case Constants.ACTION_UPDATE:
                                ...
                                break;
                            case Constants.ACTION_DELETE:
                                ClientAppartenirDelegate.deleteClientAppartenir(
                                    client
                                );
                                break;
                            default:
                        ...
                        if (vente.getCount() == vente.getCounter()) {
                            MainUI.notifySync(
                                "Kazisafe-Synchronisation",
                                vente.getCounter() +
                                    " Vente a ete enregistrer a " +
                                    vente.getRegion() +
                                    " via " +
                                    pref.get("operator", "un utilisateur"),
                                "Synchonisation"
                            );
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Vente exist = VenteDelegate.findVente(
                                    vente.getUid()
                                );
                                if (exist == null) {
                        ...
                        Vente isExist = VenteDelegate.findVente(
                            ligv.getReference().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        Produit isExist1 = ProduitDelegate.findProduit(
                            ligv.getProductId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        Mesure isExist2 = MesureDelegate.findMesure(
                            ligv.getMesureId().getUid()
                        );
                        if (isExist2 == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                LigneVente exist =
                                    LigneVenteDelegate.findLigneVente(
                                        ligv.getUid()
                                    );
                                if (exist == null) {
                        ...
                        CompteTresor isExist =
                            CompteTresorDelegate.findCompteTresor(
                                tr.getTresorId().getUid()
                            );
                        if (isExist == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                Traisorerie exist =
                                    TraisorerieDelegate.findTraisorerie(
                                        tr.getUid()
                                    );
                                if (exist == null) {
                        ...
                        Depense isExist = DepenseDelegate.findDepense(
                            opera.getDepenseId().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        Traisorerie isExist1 =
                            TraisorerieDelegate.findTraisorerie(
                                opera.getCaisseOpId().getUid()
                            );
                        if (isExist1 == null) {
                            return;
                        }
                        if (opera.getCount() == opera.getCounter()) {
                            if (!opera.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Une depense a ete effectuee chez " +
                                        opera.getAction(),
                                    "Synchonisation"
                                );
                            }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                Operation exist =
                                    OperationDelegate.findOperation(
                                        opera.getUid()
                                    );
                                if (exist == null) {
                            ...
                            case Constants.ACTION_UPDATE -> OperationDelegate.updateOperation(
                                opera
                            );
                            case Constants.ACTION_DELETE -> OperationDelegate.deleteOperation(
                                opera
                            );
                            default -> {
                            }
                        ...
                        if (opera.getCount() == opera.getCounter()) {
                            if (!opera.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Une compte depense a ete cree a " +
                                        opera.getRegion(),
                                    "Synchonisation"
                                );
                            }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                Depense exist = DepenseDelegate.findDepense(
                                    opera.getUid()
                                );
                                if (exist == null) {
                            ...
                            case Constants.ACTION_UPDATE -> DepenseDelegate.updateDepense(
                                opera
                            );
                            case Constants.ACTION_DELETE -> DepenseDelegate.deleteDepense(
                                opera
                            );
                            default -> {
                            }
                        ...
                        if (opera.getCount() == opera.getCounter()) {
                            if (!opera.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Un compte de tresorerie de la region " +
                                        opera.getRegion() +
                                        " a ete cree",
                                    "Synchonisation"
                                );
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE:
                                ...
                                if (exist == null) {
                                    List<CompteTresor> cpts =
                                        CompteTresorDelegate.findByNumeroCompte(
                                            opera.getNumeroCompte()
                                        );
                                    if (cpts.isEmpty()) {
                        ...
                        String action = opera.getAction();
                        ClientOrganisation isExist =
                            ClientOrganisationDelegate.findClientOrganisation(
                                opera.getOrganisId().getUid()
                            );
                        if (isExist == null) {
                            return;
                        }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                Facture exist = FactureDelegate.findFacture(
                                    opera.getUid()
                                );
                                if (exist == null) {
                            ...
                            case Constants.ACTION_UPDATE -> FactureDelegate.updateFacture(
                                opera
                            );
                            case Constants.ACTION_DELETE -> FactureDelegate.deleteFacture(
                                opera
                            );
                            default -> {
                            }
                        ...
                        Recquisition isExist =
                            RecquisitionDelegate.findRecquisition(
                                opera.getRecquisitionId().getUid()
                            );
                        if (isExist == null) {
                            return;
                        }
                        Destocker isExist1 = DestockerDelegate.findDestocker(
                            opera.getDestockerId().getUid()
                        );
                        if (isExist1 == null) {
                            return;
                        }
                        ...
                        if (opera.getCount() == opera.getCounter()) {
                            if (!opera.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Une marchandise recquisitionee a ete retorunee au depot depuis " +
                                        opera.getRegion(),
                                    "Synchonisation"
                                );
                            }
                        ...
                        switch (action) {
                            case Constants.ACTION_CREATE -> {
                                RetourDepot exist =
                                    RetourDepotDelegate.findRetourDepot(
                                        opera.getUid()
                                    );
                                if (exist == null) {
                                    RetourDepotDelegate.updateRetourDepot(
                                        opera
                                    );
                                }
                            }
                            case Constants.ACTION_UPDATE -> RetourDepotDelegate.updateRetourDepot(
                                opera
                            );
                            case Constants.ACTION_DELETE -> RetourDepotDelegate.deleteRetourDepot(
                                opera
                            );
                            default -> {
                            }
                        ...
                        LigneVente isExist = LigneVenteDelegate.findLigneVente(
                            opera.getLigneVenteId().getUid()
                        );
                        if (isExist == null) {
                            return;
                        }
                        Client isExist2 = ClientDelegate.findClient(
                            opera.getClientId().getUid()
                        );
                        if (isExist2 == null) {
                            return;
                        }
                        if (opera.getCount() == opera.getCounter()) {
                            if (!opera.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Une Marchandise vendue a ete retournee au magasin " +
                                        opera.getRegion(),
                                    "Synchonisation"
                                );
                            }
                        ...
                        if (action.equals(Constants.ACTION_CREATE)) {
                            RetourMagasin exist =
                                RetourMagasinDelegate.findRetourMagasin(
                                    opera.getUid()
                                );
                            if (exist == null) {
                                RetourMagasinDelegate.updateRetourMagasin(
                                    opera
                                );
                            }
                        ...
                        if (opera.getCount() == opera.getCounter()) {
                            if (!opera.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    "Kazisafe-Synchronisation",
                                    " Une facture a ete marquee comme a retirer ",
                                    "Synchonisation"
                                );
                            }
                        }
                        uid = opera.getUid();
                        if (action.equals(Constants.ACTION_CREATE)) {
                            Aretirer exist = AretirerDelegate.findAretirer(
                                opera.getUid()
                            );
                            if (exist == null) {
                        ...
                        byte[] pixcha = DatatypeConverter.parseBase64Binary(
                            photoB64
                        );
                        ...
                        if (!abn.getFrom().equals(userid)) {
                            MainUI.notifyConnect(
                                null,
                                "Connexion a Kazisafe",
                                abn.getAgent() +
                                    " vient de se connecter à son compte Kazisafe",
                                6
                            );
                        }
                        if (typeAb.equalsIgnoreCase("Gold")) {
                            ...
                            MainUI.notifySync(
                                "Kazisafe-Abonnement",
                                "Abonnement Bonus " + typeAb + " active",
                            ...
                            if (!abn.getFrom().equals(userid)) {
                                MainUI.notifySync(
                                    ...
                                    abo +
                                        " vient de se connecter à son compte Kazisafe",
                                    "Notification de connexion kazisafe"
                                );
                            }
                        }
    ...
    public void onClose(Session session, CloseReason closeReason) {
        ...
        if (onWebsocketCloseListener != null) {
            onWebsocketCloseListener.onWebSocketClose(
                session,
                !session.isOpen()
            );
        }
    ...
    public void setOnWebsocketCloseListener(
        OnWebsocketCloseListener onWebsocketCloseListener
    ) {
